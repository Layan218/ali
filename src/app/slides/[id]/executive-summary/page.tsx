...
type PresentationRow = {
  ...
  status: "Draft" | "Final";
  comments: number;
  _updatedAt?: Date | null; // Temporary field for sorting
};
    ...
    const unsubscribe = onSnapshot(
      ...
      async (snapshot) => {
        ...
        loadedPresentations.sort((a, b) => {
          const aTime = a._updatedAt?.getTime() || 0;
          const bTime = b._updatedAt?.getTime() || 0;
          return bTime - aTime;
        });

        // Remove temporary sorting field
        const finalPresentations: PresentationRow[] = loadedPresentations.map(({ _updatedAt, ...rest }) => rest);
